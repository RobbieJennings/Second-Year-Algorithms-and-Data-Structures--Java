...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    private int height(Node x, int n) {
    	if(x.left != null) {
    		if(x.right != null) {
    			if(x.right.N > x.left.N ) return height(x.right, n+1);
    			else return height(x.left, n+1);
    		}
    		else return height(x.left, n+1);
    	}
    	else if(x.right != null) return height(x.right, n+1);
    	return n;
    }
    ...
    public Key median() {
    	...
    	return median(root, ((root.N + 2)/2));
    }
    
    private Key median(Node x, int n) {
    	if(x.N > n) {
    		if(x.left.N >= n) return median(x.left, n);
    		else return median(x.right, n-x.left.N-1);
    	}
    	else return x.key;
    }
    ...
    private String printKeysInOrder(Node x) {
    	...
    	if(x.left != null) keys += printKeysInOrder(x.left);
    	keys += ")" + x.key.toString() + "(";
    	if(x.right != null) keys += printKeysInOrder(x.right);
    	keys += ")";
    	return keys;
    }
    ...
    public void delete(Key key) {
    	if(!isEmpty()) root = delete(key, root);
    }
    ...
    private Node delete(Key key, Node x) {
         ...
         if (cmp < 0) {
        	 ...
        	 if(x.left != null) {
				 if(x.right != null) {
	        		 Node predecessor = getMax(x.left);
	        		 if(x.left.right != null) removeMax(x.left);
	        		 else x.left = x.left.left;
        	 ...
        	 else if(x.right != null) return x.right;
        	 else  return null;
         }
    }
